...
emul['Teff'] = [int(re.findall('[0-9]+',i)[0]) for i in emul['ID']]
emul['lgf'] = [int(re.findall('[0-9]+',i)[1])/100 for i in emul['ID']]
emul['vsini'] = [int(re.findall('[0-9]+',i)[2]) for i in emul['ID']]
emul['R'] = [int(re.findall('[0-9]+',i)[3]) for i in emul['ID']]
...
im = ax.scatter(table_i['FW34Hb']-table_i['FW14Hb'],5.39-table_i['lgf'],
                c=table_i['vsini'],cmap='gnuplot_r',alpha=0.7)
...
ax.figure.savefig(maindir+'tmp_plots/Emul_Fig3.png',format='png',dpi=300)
...
ax3b.figure.savefig(maindir+'tmp_plots/Emul_Fig4.png',format='png',dpi=300)
