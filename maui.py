...
def maui_results_new(input_list, solution_dir='server', check_best=True, last_only=False,
...
def maui_results(input_table='MAUI_ver11.txt', solution_dir='server', check_best=True,
    ...
    if pdfplots == True:
        ...
        from matplotlib.backends.backend_pdf import PdfPages
        pp = PdfPages(maindir + 'plots/MAUI/%s.pdf' % input_table.split('.')[0])
    ...
    for row,i in zip(table,range(len(table))):
        ...
        if check_best == True and filename != best_SNR.filename[:-5]:
            print('\nWARNING: %s does not match with best spectrum available.' % filename[:-5])
        ...
        data_row.extend([grid['Grid_name'][0]])
