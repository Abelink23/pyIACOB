...
def ib_input(table='IACOB_O9BAs_SNR20.fits',output_name='input_IB.txt'):
    ...
    with open(maindir+'tables/input_IB_new.txt','w') as file1,\
        ...
        for row,j in zip(table,range(len(table))):
            ...
            match_IB = input_IB[input_IB['Name'] == name]
            ...
            if len(match_IB) == 1:
                    ...
                    if star.file_name == match_IB['spectrum']:
                        # If it has been processed, add "#"
                        if not search(name+'_'+match_IB['line'][0]+'.ps',ibdir) == None:
                            idx = '#'+str(j)
                        else:
                            idx = str(j)
                        ...
                        (idx,match_IB['Name'][0],match_IB['path'][0],match_IB['spectrum'][0],
            ...
            if len(match_IB) == 0 or (len(match_IB) == 1 and not star.file_name == match_IB['spectrum']):
                file1.write('%i %s %s %s %i %s\n' % \
                (i,star.name_star,star.spectrum.split(star.file_name)[0],\
                star.file_name,star.resolution,line))
                i = i + 1

                # ...and add it to the new main list.
                file2.write('%s %s %s %s %i %s\n' % \
                (str(j),star.name_star,star.spectrum.split(star.file_name)[0],\
                star.file_name,star.resolution,line))
...
def gen_table(input_table='input_IB.txt',check_best=True,format='fits'):
    ...
    for row,i in zip(table,range(len(table))):
        name_star = row['Name']
        file_name = row['spectrum']
        line = row['line']

        match = []
        for file in os.listdir(ibdir):
            if file.endswith('_resul.xdr') and file.split('_')[0] == name_star:
                match.append(ibdir+file)

        if len(match) == 0:
            print('\nWARNING: No .xdr file found for %s. Continuing...' % name_star)
            continue
        elif len(match) > 1:
            print('\nWARNING: More than one .xdr (line) file found for %s:' % name_star)
            print([xdr.split('%s_' %name_star)[1].split('_resul.xdr')[0] for xdr in match])
        ...
        if check_best == True and file_name != spec(name_star,SNR='best').file_name:
